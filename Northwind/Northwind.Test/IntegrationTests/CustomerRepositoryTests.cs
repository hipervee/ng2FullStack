...
using Northwind.Repository.Queries;
...
namespace Northwind.Test.IntegrationTests
{
    ...
    public class CustomerRepositoryTests
    {
        ...
        [TestMethod]
        public void GetCustomersInDallas()
        {
            using (IDataContextAsync context = new NorthwindContext())
            using (IUnitOfWorkAsync unitOfWork = new UnitOfWork(context))
            {
                IRepositoryAsync<Customer> customerRepository = new Repository<Customer>(context, unitOfWork);
                var fromMexico = new CustomerLogisticsQuery().FromCountry("Mexico");

                var customerList = customerRepository.Query(fromMexico).Select().ToList();
    
                Assert.IsNotNull(fromMexico);
            }
        }
    }
}